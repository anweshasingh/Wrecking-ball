// gaving name spaceing. Engine it's like universe. World is like earth. Bodies are like living and non-living bodies on the earth.
const Engine = Matter.Engine;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Constraint = Matter.Constraint;
// name of our engine
var engine1;
// name of our world
var world1;
// name for ball and ground
var ground;
var box1,box2,box3,box4,box5,box6,box7,box8,box9,box10,box11,box12,box13,box14,box15;
var ball;
var rope;

function setup() {
  createCanvas(1000,500);
engine1 = Engine.create(); // creating engine1 (universe)
world1 = engine1.world; // creating world1 inside engine1 (Earth)

box1 = new Box(750, 200, 50,50);
box2 = new Box(750, 150, 50,50);
box3 = new Box(750, 100, 50,50);
box4 = new Box(750, 50, 50,50);
box5 = new Box(750,  0, 50,50);
box6 = new Box(750, 200, 50,50);
box7 = new Box(750, 200, 50,50);
box8 = new Box(750, 200, 50,50);

box9 = new Box(800, 200,50,50);
box10 = new Box(800, 150,50,50);
box11 = new Box(800, 100,50,50);
box12 = new Box(800, 50,50,50);
box13 = new Box(800, 0,50,50);
box14 = new Box(800, 200,50,50);
box15 = new Box(800, 200,50,50);

box16 = new Box(850, 200,50,50);
box17 = new Box(850, 150,50,50);
box18 = new Box(850, 100,50,50);
box19 = new Box(850, 50,50,50);
box20 = new Box(850, 0,50,50);
box21 = new Box(850, 200,50,50);
box22 = new Box(850, 200,50,50);
box23 = new Box(850, 200,50,50);


ball = new Ball(500,400, 30,30);
rope = new Rope(ball.body, {x:700, y:50});
ground = new Ground(500, 490, 1000, 10);

}

function draw() {
  background(0,0,0);  
  Engine.update(engine1); // starting engine 
 
  box1.display();
  box2.display();
  box3.display();
  box4.display();
  box5.display();
  box6.display();
  box7.display();
  box8.display();

  box9.display();
  box10.display();
  box11.display();
  box12.display();
  box13.display();
  box14.display();
  box15.display();

  box16.display();
  box17.display();
  box18.display();
  box19.display();
  box20.display();
  box21.display();
  box22.display();
  box23.display();

  ball.display();
  rope.display();
  ground.display();
}

function mouseDragged() {
  Matter.Body.setPosition(ball.body, {x: mouseX, y:mouseY});
}